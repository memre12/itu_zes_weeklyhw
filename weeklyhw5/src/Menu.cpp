...
void Menu::eraseItems(const int& index) {
    if (index >= 0 && index <= chosenitems.size()) {
        // Print the item to be removed
        std::cout << "Removing item " << ": " << chosenitems[index].get_Foodname() << std::endl;

        // Remove the element at the specified index
        chosenitems.erase(chosenitems.begin() + index);
...
MenuItem::MenuItem(std::string name, double food_nameobj, std::vector<int> taste_balanceobj) : food_name(name), foodprice(food_nameobj), taste_balance(taste_balanceobj) {
    ...
    bool clampedValues = std::any_of(taste_balance.begin(), taste_balance.end(), [](int value)->bool {
